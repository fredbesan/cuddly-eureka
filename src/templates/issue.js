...
const Issue = ({ data, location }) => {
    ...
    const tabs = [
        { active: true, text: `Qué` },
        { active: true, text: `Por qué` },
        { active: false, text: `Cuando` },
        { active: false, text: `Dónde` },
        { active: false, text: `Quién` },
        { active: false, text: `Cómo` },
    ]
    ...
    return (
        <>
            <MetaData
                ...
                type="article"
            />
            <Layout heroBody={HeroBody} tabs={tabs}
                navColor="white"
                title={post.title}
                excerpt={post.excerpt}
                host={post.primary_author.name}
                modifiers={`is-medium has-text-centered is-primary`}>
...
const HeroBody = ({title, excerpt, host}) => (
    <>
        <h1 className="title is-spaced big-title has-text-white	">{title}</h1>
        <h2 className="subtitle is-4-desktop is-5-mobile">{excerpt}</h2>
        <h2 className="subtitle is-size-6-desktop is-size-5-mobile">Hosted by <strong>{host}</strong></h2>
    </>
)
